...
using Graph = std::vector<std::map<uint32_t, uint32_t>>; // neighbour, distance
...
Graph SpanningTree() {
    ...
    while (!edges.empty()) {
        while (!edges.empty()) {
            ...
            mb[edge.second.first].insert({edge.second.second, edge.first});
            mb[edge.second.second].insert({edge.second.first, edge.first});
...
uint32_t CountSubtree(const Graph& tree, uint32_t v, uint32_t p) {
    uint32_t cnt = 0;

    for (auto u : tree[v]) {
        if (u.first != p) {
            cnt += u.second;
            cnt += CountSubtree(tree, u.first, v);
        }
    }
...
void FindCentroid(const Graph& tree) {
    std::set<std::pair<uint32_t, uint32_t>> sub_weight; // weight, vertex;

    for (uint32_t v = 1; v <= vertex_amount; v++) {
        uint32_t cur = 0;
        for (auto u : tree[v]) {
            cur = std::max(cur, CountSubtree(tree, u.first, v));
        }

        sub_weight.insert({cur, v});
    }

    for (auto& w : sub_weight) {
        std::cout << naming[w.second] << " - " << w.first << "\n";
    }
}
...
int main() {
    ...
    LoadGraph();
    LoadDistances();
    Graph spanning_tree = SpanningTree();
    FindCentroid(spanning_tree);
    return 0;
}
